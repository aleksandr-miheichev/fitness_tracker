...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        return (
            (self.SPEED_MULTIPLIER * self.get_mean_speed() - self.SPEED_SUBTRAHEND)
            * self.weight
            / self.M_IN_KM
            * self.duration
            * self.MIN_IN_HOUR
...
class Swimming(Training):
    ...
    def get_mean_speed(self) -> float:
        """Рассчитывает среднюю скорость при тренировке: плавание."""
        return self.length_pool * self.count_pool / self.M_IN_KM / self.duration

    def get_spent_calories(self) -> float:
        """Рассчитывает количество потраченных ккал за тренировку: плавание."""
        return (
            (self.get_mean_speed() + self.SPEED_TERM)
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    if workout_type not in DATA_MATCHING:
        raise ValueError(WORKOUT_FAIL.format(workout_type))
